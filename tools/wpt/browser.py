...
class Safari(Browser):
    ...
    def _download_image(self, downloads, dest, system_version=None):
        ...
        self.logger.info(f"Downloading Safari from {url}")
        resp = get(url)
        ...
        if not filename:
            filename = "SafariTechnologyPreview.dmg"
    ...
    def _download_extract(self, image_path, dest, rename=None):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pkgs = [p for p in os.listdir(mountpoint) if p.endswith(".pkg")]
            if len(pkgs) != 1:
                raise ValueError(
                    f"Expected a single .pkg, found {len(pkgs)}: {', '.join(pkgs)}"
                )
    ...
    def install(self, dest=None, channel=None):
        # We can't do this either because stable/beta releases are system components or
        # because it requires admin permissions.
        raise NotImplementedError
